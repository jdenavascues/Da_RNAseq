...
tryCatch(
  { readRDS(paste0(motifpath, "/motif_annotations_dm6_v8.RDS")) },
  error = function(e) {
    message(e)
...
tryCatch(
  { readRDS(paste0(motifpath, "/motif_rankings_dm6_v8.RDS")) },
  error = function(e) {
    message('\n"motif_rankings_dm6_v8.RDS" is not available locally -- it will be downloaded and saved.\n\n')
    message(e)
    options(timeout = max(1500, getOption("timeout"))) # number is in seconds, and may depend on the connection
    temp <- tempfile()
    download.file(rk_v8_url, temp, mode='wget')
    motif_rankings_dm6_v8 <- importRankings(temp)
    #unlink(temp)
    saveRDS(motif_rankings_dm6_v8, paste0(motifpath, "/motif_rankings_dm6_v8.RDS"))
    }
)
tryCatch(
  { readRDS(paste0(motifpath, "/motif_annotations_dm6_v10.RDS")) },
  error = function(e) {
    message(e)
...
tryCatch(
  { readRDS(paste0(motifpath, "/motif_rankings_dm6_v10.RDS")) },
  error = function(e) {
    ...
    temp <- tempfile()
    download.file(rk_v10_url, temp, mode='wget')
    motif_rankings_dm6_v10 <- importRankings(temp)
    unlink(temp)
...
```{r}
motif_rankings_dm6_v10.RDS <- importRankings(
  paste0(motifpath,'/dm6_v10_clust.genes_vs_motifs.rankings.feather'))
```
...
motif.table <- data.frame(
  motif_annot = c(length(unique(motif_rankings_dm6_v8$motif)), length(unique(motif_rankings_dm6_v10$motif))),
  motif_ranks = c(nrow(motif_annotations_dm6_v8), nrow(motif_annotations_dm6_v10))
)
