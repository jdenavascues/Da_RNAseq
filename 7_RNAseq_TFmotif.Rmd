...
```{r dotplot_NES_TFclass_down, fig.width=4, fig.height=4}
palette <- c( brewer.pal(12, 'Paired')[c(8, 6, 2, 4, 10)], '#CCCCCC' )
q <- ggplot(mots_dn, aes(x=geneSet, y=NES, fill=TFcla6, colour=NULL)) + 
  geom_dotplot(binaxis='y',
               stackdir='center',
               stackgroups = TRUE,
               method='histodot',
...
```{r}
r <- ggplot(mots_up, aes(x=index, y=NES, fill=TFcla6, colour=TFcla6)) +
