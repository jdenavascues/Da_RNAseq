...
tryCatch(
  ...
  error = function(e) {
    ...
    motif_rankings_dm6_v8 <- importRankings(paste0(motifpath, '/dm6-5kb-upstream-full-tx-11species.mc8nr.genes_vs_motifs.rankings.feather'))
...
```{r}
note.tfs <- c('da', 'sc', 'ase')
for (tf in note.tfs) {
  mots_up <- mots_up %>%
    mutate('{tf}' := if_else(str_detect(mots_up$TFs, tf), tf, '') )
}
```
```{r}
da <- if_else(mots_up$da=='da', 1, 0)
da
dapeaks <- erode(da, c(0,1,0)) + erode(da, c(0,1,1)) + erode(da, c(1,1,1))
da[-c(which(diff(sign(diff(dapeaks)))==-2)+1)] <- 0
da
```
...
p <- ggplot(mots_up, aes(x=geneSet, y=NES, fill=TFcla6, colour=NULL)) + 
...
```
```{r}
# https://stackoverflow.com/questions/44991607
bw <- 10
...
p <- ggplot(mots_up, aes(x=index, y=NES, fill=TFcla6, colour=TFcla6)) +
  geom_segment(aes(x = index, xend = index,
                   y = 0, yend = NES), linewidth=0.1) +
  geom_point(size = 1, shape=21, stroke=0)
p +
  scale_colour_manual(name='',
                      breaks=TFcla6,
                      values=palette,
                      labels=exgrid_red$conditions,
                      guide=guide_legend(nrow = r)) +
  scale_fill_manual(name='',
                    breaks=levels(g$data.survtable$strata),
                    values=palette,
                    labels=exgrid_red$conditions,
                    guide=guide_legend(nrow = r)) +
  ...
  theme(strip.text.x = element_markdown(face='bold', colour='black', size=10),
        strip.background.x = element_rect(fill=cet_pal(9, name='cbd1')[[4]],
                                          colour='white'),
