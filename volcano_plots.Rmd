...
```{r}
WT_vs_DD <- read.table(file="output/dadasc/differential_expression/Control_vs_DaDaOE.txt", header=TRUE, sep="\t")
WT_vs_DA <- read.table(file="output/daoekd/differential_expression/Control_vs_DaOE.txt", header=TRUE, sep="\t")
WT_vs_SC <- read.table(file="output/dadasc/differential_expression/Control_vs_ScOE.txt", header=TRUE, sep="\t")
WT_vs_KD <- read.table(file="output/daoekd/differential_expression/Control_vs_DaKD.txt", header=TRUE, sep="\t")
gene_list <- read.table(file="resources/gene_names.txt", header=TRUE, sep=" ")
names(gene_list)[[1]] <- 'ensemblGeneID'

tables <- list("2D" = WT_vs_DD, "da" = WT_vs_DA, "sc" = WT_vs_SC, "kd" = WT_vs_KD)
