...
librarian::shelf(
  # data manip.
  readxl, writexl, dplyr, purrr, stringr, santoku, calibrate,
  # plotting
...
markers <- read_xlsx(file.path(getwd(), 'resources', 'science.abk2432_table s2.xlsx')) %>%
  ...
  tidyr::unite(gene_symbol, contains('Markers'), sep = ', ')
...
# trim and change names to match naming/presence in DEG datasets
xmarkers <- xmarkers[!(xmarkers %in% missed)]
...
```{r colour_markers}
write_xlsx(tidyr::unnest_longer(pmg.markers, xmarkers) %>%
...
```{r}
layer.heatmap2 <- function(genehm.df, cluster=FALSE, arr=NULL) {
...
```{r, fig.width=20}
p <- layer.heatmap2(genehm.df, arr='celltype')
