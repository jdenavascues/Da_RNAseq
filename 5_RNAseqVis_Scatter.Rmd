...
# data
librarian::shelf(readxl, dplyr, purrr, stringr, santoku, calibrate, httr)
# graphics
...
Load `genes_of_interest`, which was manually curated by JdN from the literature and the markers defined by the Fly Cell Atlas paper:
```{r load_GOI}
genes_of_interest <- read_excel('resources/genes_of_interest.xlsx', sheet='core')
```

Create `xmarkers`, a list of _exclusive_ cell type markers as identifiedc/curated by the Fly Cell Atlas consortium. This is paywalled so I downloaded it manually:
```{r}
markers <- read_xlsx(file.path(getwd(), 'resources', 'science.abk2432_table s2.xlsx')) %>%
  ...
  tidyr::unite(FCAmarkers, contains('Markers'), sep = ', ') %>%
  # make each gene name an independent string
  rowwise() %>% mutate(FCAmarkers = list(unique(str_split(FCAmarkers, ', ')[[1]])))
...
markers <- markers %>%
    mutate(exclusive = list(exclude(FCAmarkers)))
...
pmg.markers <- markers %>%
  ...
  dplyr::select(c(1,3))
names(pmg.markers)[[2]] <- 'markers'
cellcolours <- c('EE'='#CC79A7', 'EB'='#56B2E9', 'EC'='#009E73', 'ISC'='#D55E00')
...
```{r MA_plot_customisation2}

deg  <-  DaKD_deg
goi_list  <- tidyr::unnest_longer(pmg.markers, markers) %>%
  rename(gene_symbol = markers)

ggmaplot3 <- function(deg, goi_list, fc_thresh = 1.5,
                      md_label='*genotype^—^*', repulsion) {
  # deg is the output of DESeq2 with an added column of gene symbols
  # gene.symbols is that column
