...
Installed, not loaded: `svglite`, `limma`, `matrixStats`.

Loaded:
```{r, warning=FALSE, echo=FALSE}
# data
library(tibble)
library(DESeq2)         # to handle VSD object
# graphics
library(ggplot2)        # package 'svglite' needs be installed
...
```{r}
var_rawCounts <- matrixStats::rowVars(as.matrix(rawCounts), rm.na=TRUE)
...
var_vsd <- matrixStats::rowVars(assay(vsd), rm.na=TRUE)
#var_rawCounts <- var_rawCounts [ var_rawCounts < max(var_rawCounts) ]
...
### Pseudocounts without batch correction
