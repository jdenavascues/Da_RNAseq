...
# data
librarian::shelf(dplyr, purrr, reshape2, biomaRt, DescTools, santoku)
# graphics
librarian::shelf(UpSetR, VennDiagram, ggvenn, ggVennDiagram, eulerr)
# convenience
...
# extract genes with abs(log2FC)
define_regulation <- function(x, fc_thresh) {
  select_Significant_Genes <- x %>% 
    filter(padj <= 0.05) %>%
    filter(log2FoldChange %][% c(-fc_thresh, fc_thresh))
  
  select_Significant_Genes[, paste0('reg@abs(log2fc)>', fc_thresh)] <- cut(
    select_Significant_Genes$log2FoldChange, c(-Inf,0,Inf), c("DOWN", "UP")
    )
