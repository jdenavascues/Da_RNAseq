...
#### 5.1 Get the NCBI (Entrez)Gene IDs


The KEGG database works with these, so neither the _gene symbol_ nor the _Ensembl ID_ will work here.
```{r NCBI_GeneIDs}
...
- **KEGG pathways enriched in genes DOWN in _daughterless_ knock-down**:
```{r}
geneIDs <- ezlist %>%
...
scOE_rank <- make_degrank(ScOE_deg, mode='log2fc')
# GSEA
scOE_gseKEGG <- gseKEGG(geneList=scOE_rank,
                        organism='dme',
                        minGSSize    = 15,
                        maxGSSize    = 1000,
                        pvalueCutoff = 0.05,
...
- have a look at the metabolic signatures, for interpretation
...
# turn kg.gs into something one can use with clusterProfiler::GSEA and that's it.
