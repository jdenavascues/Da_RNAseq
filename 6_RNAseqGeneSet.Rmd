...
librarian::shelf(
  ...
  # plotting
  pathview,  cetcolor, enrichplot, genekitr, ggh4x, ggtheme, ggtext, ggrepel,
# convenience
  here)
```
...
gseCP_summarise <- function(gmx, gseCP_list, conditions, dvar) {
  ...
  if ( !(dvar %in% c('NES', 'padjust')) ) {
    stop("The argument `dvar` must be one of `NES` and `padjust`")
  }
...
NESdf <- gseCP_summarise(dutta_gmx, gse_list, conditions, 'NES')
...
```{r heatmaps}
# extract data from all gse objects
datacol <- 'NES'
empty <- data.frame(rep(0, length(unique(dutta_gmx$term))),
                    unique(dutta_gmx$term))
names(empty) <- c(datacol, 'ID')
# now fill it up with the available data from clusterProfiler
gsemerge <- function(gse, empty, datacol) {
  return(
    full_join(select(gse, {{datacol}}, ID), empty, by='ID') %>%
    mutate("{datacol}" := NES.x) %>%
    select({{datacol}}, ID)
  )
}
# make a list of all the mergers
gse_list <- lapply(
  list(daKD_gse_dutta@result, daOE_gse_dutta@result,
       dadaOE_gse_dutta@result, scOE_gse_dutta@result),
  function(x) gsemerge(x, empty, datacol='NES')
  )
names(gse_list) <- c('daKD', 'daOE', 'dadaOE', 'scOE')
NESdf <- gse_list %>%
  imap(.x = ., ~ set_names(.x, c(.y, "ID"))) %>%
  reduce(full_join, by='ID') %>%
  pivot_longer(cols=-c('ID'), names_to = "condition", values_to = 'NES')
