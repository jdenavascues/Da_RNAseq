...
gseCP_summarise <- function(gmx, gseCP_list, conditions, sets.as.factors, dvar) {
...
# MUTED AS IT TAKES >20 MIN TO RUN
...
- Tidy up the names of the GLAD gene sets
- Split the sub-types to make separate `layer.heatmaps`
- Maybe modify `gseCP_summary` to 'trim' the gene sets, maybe passing another column with the subtypes
- Produce all the heatmaps, and work out how to specify a fixed size ratio between the tile and the text (cowplot, ggarrange...)